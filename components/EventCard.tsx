...
export function EventCard({ event, variant = "default" }: EventCardProps) {
  ...
  if (variant === "compact") {
    return (
      <article className="group min-w-0 overflow-hidden rounded-lg bg-white/[0.035] transition hover:bg-white/[0.055]">
        <Link href={`/event/${event.id}`} className="grid grid-cols-[84px_minmax(0,1fr)]">
          <div className="relative min-h-24 overflow-hidden bg-slate-900">
            ...
            />
            <span className="absolute left-2 top-2 flex h-10 w-10 flex-col items-center justify-center rounded-md bg-[#071018]/90 text-[#f6f3ea] shadow-sm">
              <span className="text-sm font-bold leading-none">{date.day}</span>
              <span className="mt-0.5 text-[8px] font-bold uppercase text-[#00c2d1]">
                {date.month}
              </span>
            </span>
          </div>
          ...
          <div className="min-w-0 p-3">
            <div className="flex min-w-0 items-center gap-1.5">
              ...
              {event.admissionType === "FREE" ? (
                <span className="inline-flex shrink-0 items-center gap-1 rounded bg-[#00c2d1]/10 px-1.5 py-0.5 text-[10px] font-bold text-[#00c2d1]">
            ...
            <div className="mt-2 grid gap-1 text-xs text-slate-400">
              <p className="flex min-w-0 items-center gap-1.5">
                <MapPin className="h-3.5 w-3.5 shrink-0 text-[#00c2d1]" aria-hidden="true" />
                <span className="truncate">{event.venue.name}</span>
              </p>
              <p className="flex items-center gap-1.5">
                <Clock3 className="h-3.5 w-3.5 shrink-0 text-[#00c2d1]" aria-hidden="true" />
  ...
  return (
    <article
      className="group flex min-w-0 flex-col overflow-hidden rounded-lg bg-white/[0.035] transition duration-200 hover:-translate-y-1 hover:bg-white/[0.055]"
    >
      <Link
        href={`/event/${event.id}`}
      ...
      >
        ...
        <div className="absolute inset-x-0 top-0 flex items-start justify-between gap-2 p-3">
          <div className="flex flex-col items-start gap-2">
            ...
            {event.admissionType === "FREE" ? (
              <span className="inline-flex items-center gap-1.5 rounded-md bg-[#00c2d1] px-2.5 py-1 text-[11px] font-bold text-[#071018] shadow-sm">
                <TicketCheck className="h-3.5 w-3.5" aria-hidden="true" />
                Entrada gratis
              </span>
            ) : null}
            {event.isPopular ? (
              <span className="rounded-md bg-[#ff304f] px-2.5 py-1 text-[11px] font-bold text-white shadow-sm">
                Popular
              </span>
            ) : null}
          </div>
          <span className="flex h-14 w-14 shrink-0 flex-col items-center justify-center rounded-md bg-[#071018]/90 text-[#f6f3ea] shadow-lg shadow-black/25">
            <span className="text-xl font-bold leading-none">{date.day}</span>
            <span className="mt-1 text-[10px] font-bold uppercase text-[#00c2d1]">
              {date.month}
            </span>
          </span>
        </div>
      ...
      <div className="flex min-h-48 flex-1 flex-col p-4">
        <div className="min-h-16">
          <Link href={`/event/${event.id}`}>
            <h2 className="line-clamp-2 text-lg font-black leading-6 text-[#f6f3ea] transition group-hover:text-[#00c2d1]">
              {event.title}
            </h2>
          </Link>
          {artists ? (
